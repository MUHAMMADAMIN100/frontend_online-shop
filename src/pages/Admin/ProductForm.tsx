...
import { FiSave } from "react-icons/fi" // иконка сохранения
...
const ProductForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (id) {
        await axios.put(`https://backend-online-shop-vrxj.onrender.com/products/${id}`, product, {
          headers: { Authorization: `Bearer ${token}` },
        })
        setMessage("Товар обновлен!")
      } else {
        await axios.post("https://backend-online-shop-vrxj.onrender.com/products", product, {
          headers: { Authorization: `Bearer ${token}` },
        })
  ...
  return (
    <div className="bg-white shadow mx-auto mt-6 p-6 rounded-xl max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Кнопка с иконкой, единый стиль с ProductsManagement */}
